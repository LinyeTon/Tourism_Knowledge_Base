...
#   validate_doc_paths(state: ImportState) -> tuple[Path doc_path_obj, Path local_dir_path_obj]
#         1. 读取 `doc_path` 和 `local_dir`
#         2. 校验 `doc_path` 是否为空
#         3. 若 `local_dir` 为空，则写入默认输出目录
#         4. 转换为 `Path` 对象
#         5. 校验 doc 文件是否真实存在
#         6. 若输出目录不存在，则自动创建
#         7. 返回 `doc_path_obj` 与 `local_dir_obj`

# 1. doc dir路径校验和完善
@step_log("validate_doc_paths")
def validate_doc_paths(state: ImportGraphState) -> tuple[Path, Path]:
    """
    路径参数校验
    :param state:
    :return:
    ...
    # 2. 校验 `doc_path` 是否为空
    if not doc_path:
        logger.error(f"进行doc转化md,但是doc_path为空,无法继续进行!!")
        raise ValueError("进行doc转化md,但是doc_path为空,无法继续进行!!")
    # 3. 若 `local_dir` 为空，则写入默认输出目录
    if not local_dir:
        logger.warning(f"进行doc转化md,但是发现local_dir为空,我们给与默认值 项目/output")
        local_dir = PROJECT_ROOT / "output"  # Path
    # 4. 转换为 `Path` 对象
    doc_path_obj = Path(doc_path) # 后续对文件存在性校验! 有他更方便
    local_dir_obj = Path(local_dir) # 这块不报错
    # 5. 校验 doc 文件是否真实存在
    if not doc_path_obj.exists():
        logger.error(f"进行doc转化md,doc_path值:{doc_path},但是不存在对应的文件,业务无法继续!!")
        raise FileNotFoundError(f"进行doc转化md,doc_path值:{doc_path},但是不存在对应的文件,业务无法继续!!")
    # 6. 若输出目录不存在，则自动创建
    if not local_dir_obj.exists() :
        logger.warning(f"进行doc转化md,local_dir值:{local_dir_obj},但是没有对应文件夹! 我们自行创建!!")
        # parents 自动创建多层文件夹  例如: x/x/x/x  -> True会自动创建 x  x   x  x
        # exist_ok = True 当存在的时候不会创建也不会报错!
        local_dir_obj.mkdir(parents=True,exist_ok=True)
    # 7. 返回 `doc_path_obj` 与 `local_dir_obj`
    return doc_path_obj,local_dir_obj

# upload_doc_and_poll(doc_path_obj: Path) -> str
#        1. 校验 MinerU 配置是否完整
#        2. 调用 `/file-urls/batch` 申请上传地址与 `batch_id`
#        3. 使用 `Session(trust_env=False)` 上传 doc 文件
#        4. 根据 `batch_id` 轮询任务状态
#        5. 若任务成功，返回 `full_zip_url`
#        6. 若任务失败或超时，抛出异常
@step_log("upload_doc_and_poll")
def upload_doc_and_poll(doc_path_obj:Path) -> str:
    """
       minerU交互
    :param doc_path_obj:  上传文件的path对象
    :return: 返回的下载zip地址
    """
    # 1. 校验 MinerU 配置是否完整
    ...
    try:
        with requests.Session() as session:
            ...
            put_response = session.put(upload_file_url,data=doc_path_obj.read_bytes())
            # status_code |  code
    ...
    while True:
        ...
        if result_state == "failed":
            ...
            raise RuntimeError(
                f"获取下载的zipurl地址,minerU对应服务器发生异常! 解析失败了!!业务无法继续进行了!")
        # 正在解析中.....
        logger.warning(f"{doc_path_obj.name}minerU正在解析中......")
        time.sleep(interval_time)
...
@step_log("parse_doc_to_markdown")
def parse_doc_to_markdown(state: ImportGraphState) -> ImportGraphState:
    """
    """
    # 1. doc dir路径校验和完善
    doc_path_obj , local_dir_obj = validate_doc_paths(state)
    # 2. doc上传和zip url地址获取
    zip_url = upload_doc_and_poll(doc_path_obj)
    # 3. 下载解压并返回md_path的Path的对象
    md_path_obj  = download_and_extract_markdown(zip_url, local_dir_obj, doc_path_obj.stem)
