...
from app.process.import_.agent.nodes.node_doc_to_md import node_doc_to_md
...
main_graph.add_node("node_doc_to_md",node_doc_to_md)
...
def node_entry_after(state: ImportGraphState) -> str:
    ...
    if state['is_md_read_enabled']:
        ...
        return "node_md_img"
    elif state['is_doc_read_enabled']:
        # true 是pdf
        logger.info(f"node_entry节点判断的文件{state['local_file_path']}类型 pdf,跳转到:node_doc_to_md 节点")
        return "node_doc_to_md"
    else:
        # false 都不是
        # warning
...
main_graph.add_conditional_edges("node_entry",
                                 node_entry_after ,
                                 {
                                     ...
                                     "node_doc_to_md":"node_doc_to_md",
                                     END:END
                                 })

# 4. 添加静态边
main_graph.add_edge("node_doc_to_md", "node_md_img")
